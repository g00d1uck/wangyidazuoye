...
//封装一个模块
var util=(function(){
   function hide(ele,obj,mask){
		ele.onclick=function(){
			obj.style.display='none';
			mask.style.display='none';
			}
	};
	function show(ele,obj,mask){
		ele.onclick=function(){
  ...
  function main(){
		util.hide(Don.bar_colse,Don.bar);
		util.hide(Don.log_close,Don.login_block,Don.mask);
		util.hide( videoClose,Don.videoId,Don.mask);
		util.show(Don.videoGate,Don.videoId,Don.mask);
		util.show(Don.focusb,Don.login_block,Don.mask);
		tab(tabDesign,tabProgram,'choose',20);
        tab(tabProgram,tabDesign,'choose',10);
...
function curPagePlay(){
	ajax(defaultCourseOption);
	}
curPagePlay();

//获得两个按钮并未两个按钮添加事件
	function tab(obj1,obj2,d,a){
		obj1.onclick=function(){
			obj2.className='';
			this.className=d;
			defaultCourseOption.data.type = a;
...
var bulidPageList = function(backText){
       ...
       html += '<span class="edge left">&lt;</span>';
		...
		//调整页码表 实现页码的更新
		var midPoint = Math.ceil(playLength / 1);
